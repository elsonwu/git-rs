...
fn test_diff_integration_staged_file() {
    ...
    let file_diff = &result.file_diffs[0];
    let content_found = file_diff.chunks.iter().any(|chunk| {
        chunk.lines.iter().any(|line| line.content.contains("Staged content"))
    });
    assert!(content_found);
}
...
fn test_diff_integration_modified_file() {
    ...
    let has_addition = file_diff.chunks.iter().any(|chunk| {
        chunk.lines.iter().any(|line| {
            ...
            (line.content.contains("Modified content") || line.content.contains("New line 3"))
        })
    });
    assert!(has_removal);
    assert!(has_addition);
...
fn test_diff_integration_multiple_files() {
    ...
    let binary_diff = result.file_diffs.iter().find(|d| d.path == Path::new("binary.bin")).unwrap();
...
fn test_diff_integration_cached_vs_committed() {
    ...
    if result.files_changed > 0 {
        ...
        if !file_diff.chunks.is_empty() {
            ...
            assert!(has_content, "Should have some version-related content in diff");
        }
    }
}
...
fn test_diff_integration_post_commit_behavior() {
    ...
    let working_vs_staged = DiffCommand::diff(repository.root_path(), DiffOptions::default()).unwrap();
    let cached_options = DiffOptions { 
        cached: true,
        context_lines: 3,
        show_binary: false,
    ...
    println!("Working vs Staged: {} files changed", working_vs_staged.files_changed);
    println!("Staged vs Committed: {} files changed", staged_vs_committed.files_changed);
